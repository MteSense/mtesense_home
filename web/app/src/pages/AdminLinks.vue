...
<template>
  <section class="admin-page">
    <header class="admin-heading">
      <h1>{{ t('links') }}</h1>
      <p>{{ nav.groups.length }} groups · {{ flatLinks.length }} links</p>
    </header>
    ...
    <section class="tool-panel">
      ...
      <div class="admin-list">
        <div v-for="group in nav.groups" :key="group.id" class="admin-row">
          ...
          <button class="icon-button" type="button" :title="t('save')" @click="nav.saveGroup(group)"><Save :size="18" /></button>
          <button class="icon-button danger" type="button" :title="t('delete')" @click="nav.deleteGroup(group.id)"><Trash2 :size="18" /></button>
        </div>
      </div>
    </section>
    ...
    <section class="tool-panel">
      ...
      <div class="admin-list">
        <div v-for="link in flatLinks" :key="link.id" class="admin-row link-row">
          ...
          <button class="icon-button" type="button" :title="t('save')" @click="nav.saveLink(link)"><Save :size="18" /></button>
          <button class="icon-button danger" type="button" :title="t('delete')" @click="nav.deleteLink(link.id)"><Trash2 :size="18" /></button>
        </div>
      </div>
    </section>
  </section>
